...
type buildProjectApp struct {
	projectRepository       project.Repository
	projectGroupRepository  projectGroup.Repository
	dockerfileTplRepository dockerfileTpl.Repository
}

func (app *buildProjectApp) NewBuildProjectApp() *buildProjectApp {
	return &buildProjectApp{
...
// ToList 项目列表
func (app *buildProjectApp) ToList(groupId int, clusterId int) []BuildProjectAppDto {
	...
	// 设置Docker模板名称
	lstDTO := mapper.Array[BuildProjectAppDto](lstProject)
	lstDTO = linq.FromOrder[BuildProjectAppDto, int](lstDTO).OrderByDescending(func(item BuildProjectAppDto) int {
		return item.Id
	})
