...
type IDockerDevice interface {
	...
	// Login 登陆容器仓库
	Login(dockerHub string, loginName string, loginPwd string, progress chan string, env vo.EnvVO, ctx context.Context) bool
	...
	// Build 容器构建
	Build(env vo.EnvVO, progress chan string, ctx context.Context) bool
	// Push 上传镜像
	Push(env vo.EnvVO, progress chan string, ctx context.Context) bool
}
